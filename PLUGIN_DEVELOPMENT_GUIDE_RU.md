...
- **`plugin_manager`** - Загружает и выполняет плагины из директорий:
  - `/usr/lib/gpoa/plugins/` - системные плагины
  - `gpoa_lib/gpoa_lib/frontend_plugins/` - встроенные плагины
...
class ExampleApplier(FrontendPlugin):
    ...
    def __init__(self, dict_dconf_db, username=None, fs_file_cache=None, registry_path=None):
        ...
        Args:
            ...
            registry_path (str): Переопределение пути в реестре для данных
        """
...
- **`__init__(dict_dconf_db, username=None, fs_file_cache=None, registry_path=None)`** - инициализация
...
Параметр `registry_path` позволяет переопределить путь в реестре при
прямом использовании плагина (не через `plugin_manager`):
