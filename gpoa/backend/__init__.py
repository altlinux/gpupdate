...
def backend_factory(dc, username, is_machine, no_domain = False):
    ...
    if config.get_backend() == 'samba' and not no_domain:
        ...
        log('D9', ldata)
        try:
            back = samba_backend(sc, username, domain, is_machine, config.get_local_policy_enabled())
    ...
    if config.get_backend() == 'local' or no_domain:
        log('D8')
        try:
            back = nodomain_backend(username, is_machine, config.get_local_policy_enabled())
        except Exception as exc:
            logdata = dict({'error': str(exc)})
            log('E8', logdata)
