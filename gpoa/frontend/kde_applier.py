...
class KdeApplier(applier_frontend):
    ...
    def __init__(self, storage):
        ...
        self.__module_enabled = check_enabled(
            self.storage, self.__module_name, self.__module_experimental
        )
...
class KdeApplierUser(applier_frontend):
    ...
    def __init__(self, storage, sid=None, username=None, file_cache=None):
        ...
        self.plasma_update_flag = (
            self.plasma_update.data if self.plasma_update is not None else 0
        )
...
def create_dict(
    ...
    for setting in kde_settings:
        try:
            file_name, section, value = (
                setting.keyname.split("/")[-2],
                setting.keyname.split("/")[-1],
                setting.valuename,
            )
            data = setting.data
            if file_name == "wallpaper":
                ...
                all_kde_settings.setdefault(file_name, {}).setdefault(section, {})[
                    value
                ] = data
        except Exception as exc:
            logdata = {}
...
def apply_for_wallpaper(data, file_cache, username, plasmaupdate):
    ...
    logdata = {}
    path_to_wallpaper = (
        f"{get_homedir(username)}/.config/plasma-org.kde.plasma.desktop-appletsrc"
    )
    ...
    try:
        ...
        match = re.search(
            rf"\[Containments\]\[{id_desktop}\]\[Wallpaper\]\[org\.kde\.image\]\[General\]\s+Image=(.*)",
            current_wallpaper,
        )
        if match:
        ...
        ] = f"unix:path=/run/user/{os.getuid()}/bus"  # plasma-apply-wallpaperimage
        env_path = dict(os.environ)
        env_path["PATH"] = "/usr/lib/kf5/bin:/usr/bin"
...
def call_dbus_method(file_name):
    ...
    if file_name in dbus_methods_mapping:
        ...
        try:
            ...
            dbus_object = session_bus.get_object(
                config["dbus_service"], config["dbus_path"]
            )
