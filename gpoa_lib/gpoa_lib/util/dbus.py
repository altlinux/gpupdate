...
class dbus_runner:
    ...
    def __init__(self, username=None, force=False):
    ...
    def run(self):
        if self.username:
            ...
            log('D6', logdata)
            gpupdate = 'gpupdate' if not self.force else 'gpupdate_force'
            if is_root():
            ...
            log('D11')
            gpupdate_computer = 'gpupdate_computer' if not self.force else 'gpupdate_computer_force'
            try:
                result = self.system_bus.call_blocking(self.bus_name,
                    self._object_path,
                    self.interface_name,
